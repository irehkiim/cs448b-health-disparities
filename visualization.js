...
async function loadData() {
  const response = await fetch('data/aaqol_cleaned.csv');
...
function initEthnicityAccessExplorer(d3, data) {
  ...
  // Append to DOM instead of returning
  document.getElementById('ethnicity-explorer').appendChild(container.node());
}

// ========================================
// VISUALIZATION 2: CLUSTER EXPLORER
// ========================================
function initClusterExplorer(processed, d3) {
  // ==== CONFIG ====
  ...
  // Append to DOM instead of returning
  document.getElementById('cluster-explorer').appendChild(container.node());
}

// ========================================
// VISUALIZATION 3: CARE FUNNEL SANKEY
// ========================================
function initCareFunnelSankey(data, d3) {
  // Use d3-sankey loaded from CDN
  ...
  // Append to DOM instead of returning
  document.getElementById('care-funnel').appendChild(container.node());
}
...
async function init() {
  try {
    ...
    initEthnicityAccessExplorer(d3, data);
    console.log('Ethnicity Access Explorer initialized');

    initClusterExplorer(processed, d3);
    console.log('Cluster Explorer initialized');

    initCareFunnelSankey(data, d3);
    console.log('Care Funnel Sankey initialized');

    console.log('All visualizations initialized successfully!');
  } catch (error) {
    console.error('Error initializing visualizations:', error);
    document.body.innerHTML += '<div style="color: red; padding: 20px;">Error loading visualizations. Please check the console for details.</div>';
  }
}
